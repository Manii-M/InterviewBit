 ...
 *
 /* Approach - keeep track of all the charchter has come and put them in the queue once the char frequency is > 1 remove it from the queue and check for the next possible answer with freq 1
 * TC - O(n)
 * SC - O(n)
*/
public
    ...
    String solve(String A)
    {
        ...
        for (int i = 0; i < A.length(); i++)
        {
            ...
            while (!q.isEmpty() && map.get(q.peek()) != 1)
                q.remove();
            answer.append(q.peek() == null ? '#' : q.peek());
        }
