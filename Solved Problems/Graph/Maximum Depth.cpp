...
vector<int> Solution::solve(int A, vector<int> &B, vector<int> &C, vector<int> &D, vector<int> &E, vector<int> &F)
{
    ...
    while (!q.empty())
    {
        ...
        q.pop();
        levels[top.d].insert(D[top.x]);
        if (max_dept < top.d)
            max_dept = top.d;
        vector<int> adj = list[top.x];
    ...
    {
        int l = (E[i]) % (max_dept + 1), v = F[i];
        //     cout<<l<<" - ";
        //     for (auto itr = levels[l].begin(); itr != levels[l].end(); ++itr)
        //     {
        //         cout << *itr<<" ";
        //     }
        // cout << endl;
        set<int>::iterator it = levels[l].lower_bound(v);
        if (it == levels[l].end())
            answer.push_back(-1);
        else
            answer.push_back(*it);
