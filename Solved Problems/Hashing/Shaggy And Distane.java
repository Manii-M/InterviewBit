 ...
 *
 * approach - use hashing and update the index when encounter the same element and take the minimum
 * TC - O(n)
 * SC - O(n)
*/
...
public
    int solve(ArrayList<Integer> A)
    {
        ...
        int minDistance = Integer.MAX_VALUE, i = 0;

        for (int element : A)
        {
            if (map.containsKey(element))
                minDistance = Math.min(minDistance, i - (map.get(element) - 1));
            map.put(element, i + 1);
            i++;
        }
